...
export class RecipeHelper {
    static IsRecipeRealizeable(ing: Ingredient, stock: Ingredient[], servings: number, recipeNumberOfPeople: number): Realizeable{
    ...
    static stockIngredientExists(ing: Ingredient, stock: Ingredient[], servings: number, recipeNumberOfPeople: number): boolean{
        const stockIng = stock?.find(s => IngredientComparer.TypeEquals(s, ing));
        return stockIng !== undefined
          &&
          QuantityComparer.CompareTo(
            stockIng,
            new Ingredient(ing.quantity * (servings / recipeNumberOfPeople), ing.unit, ing.type, ing.foodType, ing.favorite)
            ) >= 0;
      }
}
