...
export class ShoppingListHelper {
    static addItem(shoppingList: Array<ShoppingListItem>, item: ShoppingListItem): void {
        if (shoppingList.some((sli) =>  IngredientComparer.TypeEquals(sli.ingredient, item.ingredient))) {
            const ingToUpdate = shoppingList.find(
                (sli) => IngredientComparer.TypeEquals(sli.ingredient, item.ingredient)
                ).ingredient;
    ...
    static removeItem(shoppingList: Array<ShoppingListItem>, item: ShoppingListItem): void{
